(function () {
  ...
  // Robust fetch: tries ApiClient first, then falls back to fetch() with header auth
  const jfFetch = async (server, path, params, token) => {
    // Strategy 1: ApiClient.getJSON (works on older Jellyfin)
    ...
    // Strategy 2: fetch() with Authorization header (Jellyfin 10.10+)
